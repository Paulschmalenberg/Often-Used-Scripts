...
var host = window.location.origin.toLowerCase();
var directoryPath = location.pathname.split('/')[1];
// Main function
function createCategoryLinks() {
  ...
  var combined = optionValues.map(function (elem, i) {
    return {
      idValue: elem,
  ...
  for (var id = 0; id <= combined.length - 1; id++) {
